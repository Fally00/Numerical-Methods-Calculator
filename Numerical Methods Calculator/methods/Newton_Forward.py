...
bp = Blueprint("newton_forward", __name__)
...
def build_forward_difference_table(y_values: list[float]) -> list[list[float]]:
    n = len(y_values)
...
def newton_forward_method(params: dict) -> dict:
    try:
        ...
        p = (x_query - x_values[0]) / h
        value = table[0][0]

        steps = [
            "Using Newton Forward interpolation.",
            f"Points loaded: {len(x_values)}",
            f"h = x1 - x0 = {h:.6f}",
            f"p = (x - x0) / h = ({x_query:.6f} - {x_values[0]:.6f}) / {h:.6f} = {p:.6f}",
            f"Term 0: y0 = {table[0][0]:.6f}",
        ]
        iterations = [
            {
                "term": 0,
                "factor": 1.0,
                "difference": round(table[0][0], 6),
                "contribution": round(table[0][0], 6),
...
@bp.route("/api/newton_forward", methods=["POST"])
def handle_newton_forward():
    params = request.get_json(force=True, silent=True) or {}
    return jsonify(newton_forward_method(params))
